...
$numq=mysql_query("SELECT * FROM `hzh` where `lyyf`='$_SESSION[gldw]'");
if($_GET[page]){  
  $pageval=$_GET[page];
...
if(($_GET[ShengfenMingcheng]!="省份"&&$_GET[ShengfenMingcheng]!="")||($_GET[ChengshiMingcheng]!="地级市"&&$_GET[ChengshiMingcheng]!="")||$_GET[YiyuanId]!=""||$_GET[YishengId]!=""||$_GET[ShijianLeixing]!=""||$_GET[ShenqingZhuangtai]!=""||$_GET[ZhuanzhenZhuangtai]!=""||$_GET[juanzhuleixing]!=""||$_GET[canbaoleixing]!=""||$_GET[ZuizaoJiehunRiqi]!=""||$_GET[ZuiwanJiehunRiqi]!=""||$_GET[shqly]!=""){
...
<input type="button" value="信息导入" onclick="wshshqyym()" class="uusub2" />
                ...
                <div class="insinsins" style="width:100%;">
                  <label>预估入组日期：</label>
                  <span>
<input type="text" id="ygrzrqksh" name="ygrzrqksh" readonly="readonly" style="width: 106px" placeholder="输入开始日期" size="12" value="" class="grd-white" />-<input type="text" id="ygrzrqjsh" name="ygrzrqjsh" style="width: 106px" readonly="readonly" placeholder="输入结束日期" size="12" value="" class="grd-white" />
...
<input id="btnGuolv" type="button" value="查询" onclick="guolvwly();" class="uusub2" />
    ...
    ?>
        <input type="button" value="RCC" class="uusub" onclick="javascript:{location.href='shqglxzshq.php?lx=RCC';}" />
...
<?php        
  ...
  $sql = "select * from `hzh` where `lyyf`='$_SESSION[gldw]'";
  if($guanjiancisql!=""){
  $sql .=$guanjiancisql;
  }
...
//echo $sql;
  ...
  while($Record = mysql_fetch_array($Query_ID)){
    echo "<tr style=\"color:#1f4248; font-size:12px;\"><td align=\"center\" bgcolor=\"#FFFFFF\"><a href=\"shqxqxgy.php?id=".$Record[0]."\">查看</a></td>";

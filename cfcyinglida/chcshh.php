...
<div class="main">
		<div class="insmain">
			<div class="thislink">当前位置：<a href="shqgl.php">申请管理</a> > <?php echo $html_title;?> </div> <!--A href="shqxq.php?id=<?php echo $hzhid;?>">申请详细信息</A--> <div class="inwrap flt top">
...
<FIELDSET class="top"><LEGEND>材料符合情况</LEGEND>
...
<TABLE id=ChuciShenqingtb_cailiaofuhes  width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#cdcdcd">
  ...
  <TR style="color:#1f4248; font-weight:bold; height:30px;">
    ...
    <Td align="center" bgcolor="#FFFFFF">无效说明 </Td></TR>
<?php /*
...
$yxtjpgData = mysql_fetch_row($yxtjpgResult);
  ...
  ?>
       <tr>
        <td class="top" align="center" bgcolor="#FFFFFF">
            <SPaN id="mchid"><?php echo $clmchRecord[1];?>
                <input type="hidden" name="mch<?php echo $clmchRecord[0];?>" id="mch<?php echo $clmchRecord[0];?>" value="<?php echo $clmchRecord[0];?>" >
                <input type="hidden" name="mchnr<?php echo $clmchRecord[0];?>" id="mchnr<?php echo $clmchRecord[0];?>" value="<?php echo $clmchRecord[1];?>" >
            </SPaN>
            <?php if($clmchRecord[1]=='索坦发票原件（8瓶）'||$clmchRecord[1]=='索坦发票原件（4瓶）'||$clmchRecord[1]=='索坦发票原件（12瓶）')
                  {
                      echo "<span>输入发票号码：<input type=\"text\" name=\"fphm\" id=\"fapiaohaoma\" class=\"grd-white\" value=\"\" onblur=\"yzhfphm();\"></span><span id=\"yahjg\"></span>";
                  }
            ?>
            <input type="hidden" name="hzhxm" id="hzhxm"value="<?php echo $hzhxm;?>">
        </td>
        <td class="top center" bgcolor="#FFFFFF"><input type="checkbox" onchange="rqxz(<?php echo $clmchRecord[0];?>)" id="shd<?php echo $clmchRecord[0];?>" name="shd<?php echo $clmchRecord[0];?>" value="1" <?php if($shfshd1[$clmchRecord[0]-1]=='1'){echo "checked";}?>></td>
        <td class="top" style="font-size:11px;" bgcolor="#FFFFFF">
            <input style="WIDth: 62px" value="<?php if($shdrq1[$clmchRecord[0]-1]!=""){echo $shdrq1[$clmchRecord[0]-1];}?>" type="text"
                   onblur="rqxzshz(<?php echo $clmchRecord[0];?>)"
                   id="shdrq<?php echo $clmchRecord[0];?>" name="shdrq<?php echo $clmchRecord[0];?>" readonly>
        </td>
        <td class="top center" bgcolor="#FFFFFF"><input type="checkbox" onchange="rqxzz(<?php echo $clmchRecord[0];?>)" id="shfyx<?php echo $clmchRecord[0];?>" name="shfyx<?php echo $clmchRecord[0];?>" value="1" <?php if($shfyx1[$clmchRecord[0]-1]=='1'){echo "checked";}?>></td>
        <td class="top"  style="font-size:11px;" bgcolor="#FFFFFF">

        <input style="WIDth: 62px" value="<?php if($shhrq1[$clmchRecord[0]-1]!=""){echo $shhrq1[$clmchRecord[0]-1];}?>" type="text" id="shhrq<?php echo $clmchRecord[0];?>" name="shhrq<?php echo $clmchRecord[0];?>" readonly />

        </td>
        <td class="top center" bgcolor="#FFFFFF"><input style="WIDth: 62px" value="<?php if($shhr1[$clmchRecord[0]-1]!=""){echo $shhr1[$clmchRecord[0]-1];}?>" type="text" id="shhr<?php echo $clmchRecord[0];?>" name="shhr<?php echo $clmchRecord[0];?>" readonly /> </td>
        <td class="top center" bgcolor="#FFFFFF"><TEXTaREa style="WIDth: 100%" rows="2" cols="20" id="bzh<?php echo $clmchRecord[0];?>" name="bzh<?php echo $clmchRecord[0];?>"><?php echo $shqbzh1[$clmchRecord[0]-1];?></TEXTaREa>
         </td>
      </tr>
  <?php
    }
...
<TEXTAREA style="WIDTH: 600px; HEIGHT: 82px" class="textarea" rows="2" cols="20" name="shscl"></TEXTAREA>
</div>
<?php
}
?>
...
<input value="0" type="radio" name="shhyj" id="shhyj1" onclick="showText(0)"/><LABEL for="shhyj1">补寄材料</LABEL>
<input value="1" type="radio" name="shhyj" id="shhyj2" onclick="showText(1)"/><LABEL for="shhyj2">材料齐全</LABEL>
<script type="text/javascript">
<?php
if($hzherqshq!="2") {
    echo "var shqclzsh=11;";
} else {
  //重新入组
//pNET RCC GIST 正常需要6份材料  RCC 1+1+1(3次以上)
  if(($hzhshqbzh=='GIST'||$hzhshqbzh=='pNET'||$hzhshqbzh=='RCC')&&($hzhjzhlx=='全部'||$hzhjzhlx=='部分'||$hzhjzhlx=='原部分')){ echo "var shqclzsh=6;";}
//RCC 1+1+1(3次以下) 需要7份材料
  else if($hzhshqbzh=='RCC'&&$hzhjzhlx=='1+1+1'){echo "var shqclzsh=7;";}
  else{echo "var shqclzsh=6;";}
}
?>
...
<div id="wtgdiv" style="display:none;">
  ...
  <div style="width:980px; float:left">
    ...
    <div style="width:700px; float:left;">
      <ul>
      <li>
<textarea rows="10" cols="80" id="wtgyy" name="wtgyy" readonly></textarea>
      </li>
      </ul>
    </div>
  </div>
...
<TEXTAREA style="WIDTH: 600px; HEIGHT: 82px" class="textarea" rows="2" cols="20" name="bzhshm"></TEXTAREA>
</div>
<div class="top">
  是否需要发送短信：<input type="radio" id="xyfsdx" name="fsdx" value="">是 <input type="radio" name="fsdx" id="bxyfsdx" value="">否 <span style="color:red">*(需要发送短信请点击是，不需要发送短信则不要点击)</span>
</div>
<div class="top">
<input id="submitBtn" type="submit" value="保存" class="uusub" />
<input type="button"  onclick="javascript:{location.href='shqxq.php?id=<?php echo $hzhid;?>'}" value="返回" class="uusub2" /></div></form>
</div>
...
<div class="mindess" id="qrfsdx" style="display:none; border:0px solid #FFFFFF; clear:both;">
	<div style="position:absolute; right:15px; top:5px;"><a style="color:#FFFFFF; cursor:pointer;" onclick="qrfsdx(0)">关闭</a></div>
		<fieldset style=" background:#ffffff; margin-bottom:2px; border:0; margin-top:-5px;">
            <legend style="background:#ffffff; border:0; padding:5px;">发送短信</legend>
            <form action="http://115.28.90.3:8080/zz/send.htm" method="post" name="">
            ...
            内&nbsp;&nbsp;容：<textarea name="content" style="width:90%; height:65px;" id="fsdxid" value=""></textarea>
            <div class="top">
            <input type="submit" name="submitdxfs" value="发送短信" class="uusub"></div>
            </form>
        </fieldset>
	  </div>

...
$numq=mysql_query("SELECT * FROM `yyyshdq` where `yhszht`='1' and `yyzhdyf` in ($yhgldw) ");// group by yymch
$yzhguanjianci =0;
$guanjiancisql = "('1'='1'";
...
$guanjiancisql .= " )";
if($yzhguanjianci==1){
$numq=mysql_query("SELECT * FROM `yyyshdq` where `yhszht`='1'  and `yyzhdyf` in ($yhgldw) and".$guanjiancisql);
}
//echo $guanjiancisql;
...
include('spap_head.php');
  ...
  $tjzshsql = "select COUNT(DISTINCT sheng),COUNT(DISTINCT shi),COUNT(DISTINCT yymch),COUNT(DISTINCT zhdyshdh),COUNT(DISTINCT shqysh1),COUNT(DISTINCT shqysh2),COUNT(DISTINCT shqysh3) from `yyyshdq` where `yhszht`='1'  and `yyzhdyf` in ($yhgldw) ";
if($yzhguanjianci==1){
...
<?php        
  ...
  $sql = "select * from `yyyshdq`  where `yhszht`='1' and `yyzhdyf` in ($yhgldw) ";
if($yzhguanjianci==1){
$sql .=" and ".$guanjiancisql;
}
  $sql .= " order by sheng ASC,shi ASC,yymch ASC limit $page $pagesize ";
//echo $sql;
  $_SESSION['xgyzhdyyyshdchsql']=$sql;
  $Query_ID = mysql_query($sql);
  while($Record = mysql_fetch_array($Query_ID)){
    echo "<tr style=\"color:#1f4248; font-size:12px;\">";
    echo "<td align=\"center\" bgcolor=\"#FFFFFF\"><a href=\"xgyyshxx.php?id=".$Record[0]."\">详情</a><br/>";
    $shqyzh=0;
  $shqyzhsql = "select * from `xgyyshxmclshq` where `yshid`='".$Record[0]."' and ( `jchrq` is NULL or `jchrq`='')";
  $shqyzhQuery_ID = mysql_query($shqyzhsql);
  while($shqyzhRecord = mysql_fetch_array($shqyzhQuery_ID)){$shqyzh=1;}
    if($shqyzh==0){
      echo "<a href=\"xgyyshclshq.php?id=".$Record[0]."\">申请</a>";
    }else{
      echo "申请中";
    }
    echo "</td>";
    ...
    echo "<td align=\"center\" bgcolor=\"#FFFFFF\"><a onclick=\"qzyzh(".$Record[8].$shqyshshl.")\">".$Record[6]."</a><input type='hidden' name='zhdysh".$Record[8]."' id='zhdysh".$Record[8]."' value='".$Record[6]."'></td>";
    //echo "<td>".$Record[7]."</td>";
    echo "<td align=\"center\" bgcolor=\"#FFFFFF\">".$Record[9]." ".$Record[12]." ".$Record[15]."<input type='hidden' name='shqysh".$Record[8]."' id='shqysh".$Record[8]."' value='".$Record[9]." ".$Record[12]." ".$Record[15]."'></td>";
    ...
    echo "<td align=\"center\" bgcolor=\"#FFFFFF\">".$yyshengshi.$Record[26].$Record[20]."</td>";
...
<div class="mindess" id="qzyzh" style="width:325px;  padding-top:5px; margin:0 auto 0 -181px; display:none;">
 ...
 chooseDateNow('ZuizaoJiehunRiqi', 'ZuiwanJiehunRiqi', true, true);
        ...
        var url = "";
        function chazhao() {
        var urlguanjianci = "xgyzhdyyysh.php";
        var jlsh=0;//记录数初始等于0
//医院

...
$tjshzi=0;
//获取全部医院名称
$yymchsql="SELECT DISTINCT yymch FROM `yyyshdq`";
$yymchq=mysql_query($yymchsql);
while($yymchRecord = mysql_fetch_array($yymchq)){
  $tjshz[$tjshzi][9]=$yymchRecord[0];
  $yyidsql="SELECT id FROM `yyyshdq` where `yymch`='".$yymchRecord[0]."'";
  $yyidq=mysql_query($yyidsql);
  while($yyidRecord = mysql_fetch_array($yyidq)){
  $yyid[]=$yyidRecord[0];
  }//药房名称获取全部id
  if($yyid!=""){
...
if($ex == '2007') { //导出excel2007文档  
    ...
    header('Content-Disposition: attachment;filename="按医院统计'.date('Y-m-d').'.xlsx"');  
    ...
    header('Content-Disposition: attachment;filename="按医院统计'.date('Y-m-d').'.xls"');  
    header('Cache-Control: max-age=0');  

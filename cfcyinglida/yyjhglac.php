...
/*  $numquery="select * from `zyff` WHERE `jhshyyyshl`>'0' and `yyzht`='1' and (`fyr`='$yhyshid'";
for($i=0;$i<count($yshid);$i++)
{
  ...
  //$numquery="select * from `zyff` WHERE `jhshyyyshl`>'0' and `yyzht`='1' and `yfmch`='$yhgldw'";
  $numquery="select SUM(jhshyyyshl) from `zyff` WHERE `jhshyyyshl`>'0' and `yyzht`='1' and `yfmch`='$yhgldw'";
  ...
  {
      $dqrq=date('Y-m-d');
      $gdkphshquery="insert into `cfcyy`(`yshxm`,`fshrq`,`shl`,`zht`,`yfmch`)values('$yfid','$dqrq','$tjshl','1','$yhgldw')";
      ...
      if(!$gdkphshresult)
      {
        ...
        //1 药房，2 cfc，3 国大,4 销毁
        $query="UPDATE `zyff` SET `yyzht`='2',`yydbpc`='$getID' WHERE `jhshyyyshl`>'0' and `yyzht`='1' and `yfmch`='$yhgldw'";
        $result=mysql_query($query);
        //echo $query;
        if(!$result)
        {
          echo mysql_error();
  echo "药房无余药 <input type=\"button\"  onclick=\"javascript:{history.go(-1);}\" value=\"返回\" class=\"uusub2\" />";
        }else{
          //$tjshl=mysql_affected_rows();
          echo "成功 打包余药$tjshl 粒<input type=\"button\"  onclick=\"javascript:{history.go(-1);}\" value=\"返回\" class=\"uusub2\" />";
        }
      }
  }else{
  echo "药房无余药 <input type=\"button\"  onclick=\"javascript:{history.go(-1);}\" value=\"返回\" class=\"uusub2\" />";

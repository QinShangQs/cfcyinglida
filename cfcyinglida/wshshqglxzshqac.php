...
include('newdb.php');
if(isset($_POST['wshshqdrcode'])) {echo "post!<br/>";
    if($_POST['wshshqdrcode'] == $_SESSION['wshshqdrcode']){echo "post=session!<br/>";
//预约id
$yyid = $_POST['yyid'];
//姓名
...
//最新要求性别以下内容不填写可用录入&&$djrq!="",`djrq`,  '$djrq'
...
$lrr=$_SESSION[yhname];
    ...
    $query="UPDATE `hzh` SET `drxt`='1' WHERE `id` = '".$yyid."'";
    $result=mysql_query($query);
    if(!$result)
    {
      echo mysql_error();
    ...
    unset($_SESSION['wshshqdrcode']);               //将其清除掉此时再按F5则无效
    echo "成功！<br/>";
    ...
    echo "session=".$_SESSION['wshshqdrcode']." psot=".$_POST['wshshqdrcode'];
    echo "该操作已完成，请不要刷新本页面或重复提交表单！";
    }
}

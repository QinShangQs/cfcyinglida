...
if($bjshl>"0"){
  $query="insert into `kfrk`(`ph`,`zhwmch`,`yxrqzh`,`ywmch`,`shchdwchd`,`shyrq`,`bjshl`,`gg`,`jxxh`,`zht`,`jlshjch`)values('$ph','$zhwmch','$yxrqzh','$ywmch','$shchdwchd','$shyrq','$bjshl','$gg','$jxxh','$zht','$jlshjch')";
  $result=mysql_query($query);
  if(!$result)
  {
    echo mysql_error();
    echo "失败 写入数据库失败  <input type=\"button\"  onclick=\"javascript:{history.go(-1);}\" value=\"返回\" class=\"uusub\" />";
  }
  else{
  echo "成功！<br/>";
  echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"2; URL=kfrkgl.php\">";
  }
}else{echo "失败 请填写库存数 <input type=\"button\"  onclick=\"javascript:{history.go(-1);}\" value=\"返回\" class=\"uusub\" />";}
?>

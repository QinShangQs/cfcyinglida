...
//                    if ($qchkc != $shcpdkcshl) {
//                        echo "</br>期初库存" . $qchkc . "不等于上次盘点库存数量" . $shcpdkcshl;
//                        $zhtsd = 1;
//                        $zhtsdi++;
//                    } else if ($qchkc + $byrk - $bychk != $shjkc) {
//                        echo "</br>期初库存" . $qchkc . "加本月入库" . $byrk . "减本月出库" . $bychk . "不等于实际库存" . $shjkc;
//                        $zhtsd = 1;
//                        $zhtsdi++;
//                    } else if ($byrk != $byrkshl) {
//                        echo "</br>本月入库数量" . $byrkshl . "不等于本月入库" . $byrk;
//                        $zhtsd = 1;
//                        $zhtsdi++;
//                    } else {
//                        echo "</br>盘点无误";
...
//                                            values('$getID','$ypph','$qchkc','$byrk','$bychk','$shjkc','$zhtsd')";
                ...
                $kfkcpdisLockSql = "UPDATE `kfkcpd` SET `islock` = 1 WHERE id='$getID'";
            ...
            if ($zhtsdi > 0) {
                ...
                exit();
                /*
                 $query="UPDATE `manager` SET `yhzht`='0' WHERE `names` = '".$_SESSION[yhln]."'";
                  $result=mysql_query($query);
                  if(!$result)
                  {
                    echo mysql_error();

...
$hzhidsql="SELECT hzhid,count(hzhid) FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL group by hzhid  having count(hzhid)='".$_GET[fypsh]."' ";
$jli=0;
$guanjiancisql="(";
...
?>
  <div class="main"><?php /*div[main]开始*/?>
    <div class="insmain">
      <div class="thislink">当前位置：<a href="/manager.php">首页</a>-><a href="xgydfqdgl.php"><?php echo $html_title;?></a></div>
...
<?php 
$numsql = "select a.* from `hzh` a LEFT JOIN `xclyrq` b on a.`id`=b.`hzhid` where a.`shqzht`='入组' and b.`xclyrq`>='".date('Y-m-d',strtotime('-29 day',strtotime(date('Y-m-d'))))."' and a.`lyyf` in ($yhgldw)";
if($guanjianci1!=""){
...
$kcshshdsql="SELECT SUM(pfshl1) FROM `yfshqzy` where `shqzht`='3' and `yfmch` in ($yhgldw)";

  $kcshshdQuery_ID = mysql_query($kcshshdsql);
  while($kcshshdRecord = mysql_fetch_array($kcshshdQuery_ID)){
  $kcshshd1=$kcshshdRecord[0];
  }
$kcshfy1sql="SELECT SUM(fyshl) FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL  and `yfmch` in ($yhgldw)";
//echo $kcshfy1sql;
...
?>
          ...
          预估待发药数：<?php echo $num." ";?>瓶 当药房总库存数： <?php echo $kcshshd1-$kcshfy1;?> 瓶  <?php /*(当前药房id: <?php echo $yshid." ";?>开发期间调用)*/?> <a href="cfcdfqdglexcel.php" target="_blank">导出</a>
          </span></div>
...
for($i=0;$i<count($yfid);$i++)
{
  ...
  $sql .= ")";*/
  $sql = "select * from `hzh` a LEFT JOIN `xclyrq` b on a.`id`=b.`hzhid` where a.`shqzht`='入组' and b.`xclyrq`>='".date('Y-m-d',strtotime('-1 month',strtotime(date('Y-m-d'))))."' and a.`lyyf` in ($yhgldw)";
if($guanjianci1!=""){
$sql .=" and ".$guanjianci1;
}
  $sql .= " order by b.`xclyrq` ASC limit $page $pagesize ";
  $_SESSION[cfcygfysql]=$sql;
  //echo $sql;
  ...
  //$hzhjzhshl=$Record[26];
            ...
            //读取次数
$lynumq=mysql_query("SELECT * FROM `zyff` where  `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."'");
...
//读取药房省
      ...
      echo $hzhlyyf;
            ...
            echo $lynum;?>次/<?php
$lyshlnumq=mysql_query("SELECT SUM(fyshl) FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."'");
  while($lyshlnum = mysql_fetch_array($lyshlnumq)){if($lyshlnum[0]!=""){echo $lyshlnum[0];}else{echo "0";}}
            ?>瓶</a></td>
            <td align="center" bgcolor="#FFFFFF"><?php
            if($lynum>0){
$hzhlyjlsql="SELECT yyyf FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."' order by `id` desc limit 1";
...
/* else if($lynum=="0"&&$hzhjzhlx=="部分"){
  ...
  $ygxcfyrqq=mysql_query("SELECT ygxcfyrq FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."' order by id DESC limit 0,1");
  ...
  //echo date('Y-m-d',strtotime('-7 day',strtotime($ygxcfyrqshj)));
      ...
      if($shqyshshl==0){$shqyshshl="";}else{$shqyshshl=",".$shqyshshl;}
        echo "<a onclick=\"qzyzh(".$yyRecord[5].$shqyshshl.")\">".$yyRecord[3]." ".$yyRecord[4]."</a><input type='hidden' name='zhdysh".$yyRecord[5]."' id='zhdysh".$yyRecord[5]."' value='".$yyRecord[4]."'><input type='hidden' name='shqysh".$yyRecord[5]."' id='shqysh".$yyRecord[5]."' value='".$yyRecord[6]." ".$yyRecord[7]." ".$yyRecord[8]."'>";
            ...
            ?></a></td>
            <td  align="center" bgcolor="#FFFFFF"><?php echo $Record[34];?></td>
...
for($i=0;$i<count($yfid);$i++)
{
  ...
  $sql .= ")";*/
  $sql = "select a.* from `hzh` a LEFT JOIN `xclyrq` b on a.`id`=b.`hzhid` where a.`shqzht`='入组' and b.`xclyrq`<'".date('Y-m-d',strtotime('-1 month',strtotime(date('Y-m-d'))))."' and a.`lyyf` in ($yhgldw)";
if($guanjianci1!=""){
$sql .=" and ".$guanjianci1;
}
...
$lyshlnumq=mysql_query("SELECT SUM(fyshl) FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."'");
  while($lyshlnum = mysql_fetch_array($lyshlnumq)){if($lyshlnum[0]!=""){echo $lyshlnum[0];}else{echo "0";}}
            ...
            <td align="center" bgcolor="#FFFFFF"><?php
            if($lynum>0){
$hzhlyjlsql="SELECT fyjl FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."' order by `id` desc limit 1";
$hzhlyjlq=mysql_query($hzhlyjlsql);
  while($hzhlyjl = mysql_fetch_array($hzhlyjlq)){
    if($hzhlyjl[0]=="1"){echo "200mg*60粒/瓶";}else{echo "250mg*60粒/瓶";}
  }
            }else{
              echo $hzhshqypgg;
            }
...
<div class="mindess" id="qzyzh" style="width:325px;  padding-top:5px; margin:0 auto 0 -181px; display:none;">
  ...
  <table style="margin-top:30px;" width="100%" border="1" cellpadding="10" cellspacing="1">
    ...
    <script type="text/javascript">
        function guolv() {
            var url = 'xgydfqdgl.php?day=' + $('#Day').val();
            location.href = url;
        }
        function chazhao() {
            var url = 'xgydfqdgl.php?guanjianci=' + encodeURIComponent($('#Guanjianci').val());
            location.href = url;
        }
        function guolvpsh() {
        if($('#YifayaoZhishu').val()==0){var url = 'xgydfqdgl.php';}
        else{
            var url = 'xgydfqdgl.php?fypsh=' + encodeURIComponent($('#YifayaoZhishu').val());}
            location.href = url;
        }

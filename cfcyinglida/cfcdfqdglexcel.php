...
$k8="预估下次发药日期";  
$k9="入组医院";  
$k10="预估本次用药开始时间";  //后期调整全部姓名就行。 证件号码不要了
$k11="药房";  
...
$objExcel->getActiveSheet()->setCellValue('H2', "$k10"); 
$objExcel->getActiveSheet()->setCellValue('I2', "$k9"); 
...
$yfmchq=mysql_query(" select `yfmch`,`yfshi` from `yf` group by `yfmch`");
while($yfmchRecord = mysql_fetch_array($yfmchq)){
  $yfmchs[$yfmchRecord[0]]=$yfmchRecord[1];
}
//print_r($yfmchs);die('asdf');

$sqlchfq=explode("limit",$_SESSION[cfcygfysql]);
$sql=$sqlchfq[0];
  //$sql = "select * from `hzh`";
  $jshi=3;
...
$ztsh=$ztsh+2;
  ...
  $hzhrzjl=$Record[28];//患者入组是剂量
  $hzhlyyf=$Record[36];//患者领药药房
...
if($lynum=="0"){
  ...
  $ygxcfyrqq=mysql_query("SELECT ygxcfyrq,fyjl FROM `zyff` where `tshqk`='0' and `zyffzht` IS NULL and `hzhid`='".$Record[0]."' order by id DESC limit 0,1");
...
if($hzhfyjl==1){$hzhfyjlgg="200mg*60粒/瓶";$hzhfyjl=0;$hzhbcyykshrq=date('Y-m-d',strtotime('+8 day',strtotime($ygxcfyrq)));}else if($hzhfyjl==2){$hzhfyjlgg="250mg*60粒/瓶";$hzhfyjl=0;$hzhbcyykshrq=date('Y-m-d',strtotime('+8 day',strtotime($ygxcfyrq)));}else {$hzhfyjlgg=$hzhrzjl;$hzhbcyykshrq=$hzhygshcyyshj;}
    $objExcel->getActiveSheet()->setCellValue('f'.$jshi, $hzhfyjlgg); 
    $objExcel->getActiveSheet()->setCellValue('g'.$jshi, $ygxcfyrq);  
    $objExcel->getActiveSheet()->setCellValue('h'.$jshi, $hzhbcyykshrq);
    ...
    $objExcel->getActiveSheet()->setCellValue('i'.$jshi, $yzyy);   
    $objExcel->getActiveSheet()->setCellValue('j'.$jshi, $yfmchs[$hzhlyyf]);   
...
}$objExcel->getActiveSheet()->setCellValue('h'.$jshi, $hzhzhxqsh );*/
...
$objExcel->getActiveSheet()->getStyle('A1:J'.$ztsh)->getAlignment()->setWrapText(true);//自动换行

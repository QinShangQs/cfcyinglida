...
<!--                 </div> -->
            ...
            <div class="bodyer">
                <div class="login">
                    <form method="post" action="/i_pat_reg.php" id='formId'>
                    <table>
                        <tbody>
                            <tr>
                            ...
                            <tr>
                                <td class="one"><font color="red">*</font>密码：</td>
...
<?php
    ...
    (empty($username) || empty($password)) && $db->returnError('账号或密码不能为空', '点击返回','/i_pat_reg.php'); //
    ($password != $passwordn) && $db->returnError('两次密码不相同', '/i_pat_reg.php');
    $arr = $db->getRow("select * from patient_account where name = '" . $username . "'");
    !empty($arr) && $db->returnError('该账号已存在！', '/i_pat_reg.php');
    $arr = $db->insert('patient_account', ['name' => $username, 'password' => md5($password), 'question_id' => $question, 'question_dan' => $quedan]);
    $db->returnError('注册成功！', '前去登陆', '/i_patient_login.php');
//     header("location: i_patient_login.php");
  endif;
?>
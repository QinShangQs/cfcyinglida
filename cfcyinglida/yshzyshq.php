...
?>
    ...
    <div class="top">
        <form action="yshzyshqac.php" method="post" onsubmit="return check()">
        <div>
...
/*$pfshl1sql="SELECT SUM(pfshl1) FROM `yfshqzy` where `yshid`='".$yshid."'" ;
for($i=0;$i<count($yfid);$i++)
{
  if($yfid[$i]!=null){
    $pfshl1sql .= " or `yshid`='".$yfid[$i]."' ";
  }
}*/
$pfshl1sql="SELECT SUM(pfshl1) FROM `yfshqzy` where `shqzht`='3' and `yfmch`='".$yhgldw."'" ;

$pfshl1q=mysql_query($pfshl1sql);
while($pfshl1Record = mysql_fetch_array($pfshl1q)){$pfshl1=$pfshl1Record[0];}
/*$pfshl1ztsql="SELECT SUM(pfshl1) FROM `yfshqzy` where `shqzht`='2' and ( `yshid`='".$yshid."'" ;
for($i=0;$i<count($yfid);$i++)
{
  if($yfid[$i]!=null){
    $pfshl1sztql .= " or `yshid`='".$yfid[$i]."' ";
  }
}
$pfshl1ztsql .=")";*/
...
/*$zyffshl1sql="SELECT SUM(fyshl) FROM `zyff` where `fyjl`='1' and (`fyr`='".$yhid."'";
for($i=0;$i<count($yfyshid);$i++)
{
  if($yfyshid[$i]!=null){
    $zyffshl1sql .= " or `fyr`='".$yfyshid[$i]."' ";
  }
}
$zyffshl1sql .= ")";*/
$zyffshl1sql="SELECT SUM(fyshl) FROM `zyff` where `yfmch`='".$yhgldw."'";
$zyffshl1q=mysql_query($zyffshl1sql);
while($zyffshl1Record = mysql_fetch_array($zyffshl1q)){$zyffshl1=$zyffshl1Record[0];}
echo ($pfshl1-$zyffshl1);
if($pfshl1zt>0){echo "（另有".$pfshl1zt."在途中）";}
//echo $pfshl1."-".$zyffshl1;
            ?><input name="kcshl1" type="hidden" value="<?php echo ($pfshl1-$zyffshl1);?>" />
</div>
        <div class="top">
            <span class="label">申请药品数（盒）：</span><input class="grd-white" id="shqshouceshl" name="shqshl1" style="width: 460px;" type="text" value="" /></div>
<div class="top"></div>
 <div class="top">
  <!-- 申请手册数量：<input type="text" name="shouceshl" class="grd-white" value="" id="shqshouceshl" onblur="shqshcshl();"> -->
        ...
        <div class="top">
            <input type="submit" class="uusub" value="保存" /> <input type="button"  onclick="javascript:{history.go(-1);}" value="返回" class="uusub2" /></div>
